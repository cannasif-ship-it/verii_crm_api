    ...
    public class PdfReportTemplateService : IPdfReportTemplateService
    {
        ...
        public async Task<ApiResponse<PdfReportTemplateDto>> CreateAsync(CreatePdfReportTemplateDto dto, long userId)
        {
            ...
            try
            {
                ...
                await _unitOfWork.SaveChangesAsync().ConfigureAwait(false);
                await _pdfTemplateAssetService.NormalizeTemplateAssetPathsAsync(dto.TemplateData, userId, template.Id, dto.RuleType).ConfigureAwait(false);
                template.TemplateJson = JsonSerializer.Serialize(dto.TemplateData, PdfReportTemplateJsonOptions.CamelCase);
