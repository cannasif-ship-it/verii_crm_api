    ...
    public class PdfReportDocumentGeneratorService : IPdfReportDocumentGeneratorService
    {
            ...
            Action<string> onReject)
        {
            ...
            foreach (var el in imageElements)
            {
                ...
                {
                    var resolutionBasePath = ResolveLocalImageBasePath(key);
                    if (string.IsNullOrEmpty(resolutionBasePath))
                    {
                        _logger.LogWarning("PdfReportDocumentGenerator local image skipped (base path not configured): Path={Path}", key);
                    ...
                    try
                    {
                        var sanitized = NormalizeRelativeImagePath(key);
                        ...
                        var baseFull = Path.GetFullPath(resolutionBasePath);
                        var fullPath = Path.GetFullPath(Path.Combine(resolutionBasePath, sanitized));
                        ...
                        var localBytes = await File.ReadAllBytesAsync(fullPath).ConfigureAwait(false);
                ...
                try
                {
                    using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(_options.ImageFetchTimeoutSeconds));
        ...
        private string? ResolveLocalImageBasePath(string source)
        {
            if (source.StartsWith("/uploads/", StringComparison.OrdinalIgnoreCase))
            {
                if (!string.IsNullOrWhiteSpace(_options.LocalImageBasePath))
                    return Path.Combine(_options.LocalImageBasePath, "uploads");

                return null;
            }

            return string.IsNullOrWhiteSpace(_options.LocalImageBasePath)
                ? null
                : _options.LocalImageBasePath;
        }

        private static string NormalizeRelativeImagePath(string source)
        {
        ...
        {
            var lines = ResolvePropertyPathStatic(entityData, "Lines") as IEnumerable;
            if (lines == null)
                return 20;
