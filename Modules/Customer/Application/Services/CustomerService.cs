    ...
    public class CustomerService : ICustomerService
    {
        ...
        public async Task<ApiResponse<PagedResponse<CustomerGetDto>>> GetAllCustomersAsync(PagedRequest request)
        {
            try
            {
                ...
                var sortBy = request.SortBy ?? nameof(CustomerEntity.Id);
        ...
        private static List<CustomerDuplicateCandidateDto> BuildDuplicateCandidates(
            ...
            Func<CustomerEntity, string> keySelector,
            string matchType,
            decimal score)
        {
        ...
        private async Task<(CustomerEntity Customer, string Action, bool ShouldRefreshCoordinates)> ResolveBusinessCardCustomerAsync(CustomerCreateFromMobileDto request)
        {
            ...
            IQueryable<CustomerEntity> query = _unitOfWork.Customers.Query(tracking: true, ignoreQueryFilters: true);
        ...
        private bool MergeBusinessCardCustomer(CustomerEntity customer, CustomerCreateFromMobileDto request, bool overwriteExistingFields)
        {
        ...
        private async Task TryFillCoordinatesFromAddressAsync(CustomerEntity customer, bool allowOverwriteExistingCoords = false)
        {
